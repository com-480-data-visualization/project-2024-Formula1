...
const svg = d3.select("#map").append("svg")
    ...
    .style("background-color", "#080b0c") // Water color
...
const zoom = d3.zoom().on("zoom", function (event) {
    ...
    g.selectAll("circle").attr("r", d => Math.sqrt(d.races) * 2 / event.transform.k);
    g.selectAll(".country-label").style("font-size", `${10 / event.transform.k}px`);
    g.selectAll(".country").style("stroke-width", `${1 / event.transform.k}px`);
});
...
d3.json("https://unpkg.com/world-atlas@1.1.4/world/110m.json").then(world => {
    ...
    g.selectAll(".country")
        ...
        .style("stroke", "#080b0c") // Border color
    ...
    d3.json("data/circuits.json").then(data => {
        ...
        circles.on("mouseover", (event, d) => {
            ...
            d3.selectAll(".bar").transition().duration(100).style("opacity", 0.2);
            d3.select(`.bar-${d.continent.replace(/\s+/g, '-')}`).transition().duration(100).style("opacity", 1);
            // Highlight the corresponding area
            d3.selectAll(".area").transition().duration(100).style("opacity", 0.2);
            d3.select(`.area-${d.continent.replace(/\s+/g, '-')}`).transition().duration(100).style("opacity", 1);
        })
        .on("mouseout", d => {
            tooltip.transition()
                .duration(100)
            ...
            d3.selectAll(".bar, .area").transition().duration(100).style("opacity", 0.8);
...
function createStackedBarChart(data, circles) {
    ...
    barSvg.selectAll(".bar")
        ...
        .on("mouseover", (event, d) => {
            ...
            circles.transition().duration(200).style("opacity", 0.2);
            ...
            d3.selectAll(".bar, .label").transition().duration(200).style("opacity", 0.2);
            d3.selectAll(`.bar-${d.continent.replace(/\s+/g, '-')}, .label-${d.continent.replace(/\s+/g, '-')}`)
                .transition().duration(200).style("opacity", 1);

            // Highlight the corresponding area
            d3.selectAll(".area").transition().duration(200).style("opacity", 0.2);
            d3.selectAll(`.area-${d.continent.replace(/\s+/g, '-')}`).transition().duration(200).style("opacity", 1);
        })
        .on("mouseout", d => {
            // Reset opacity of circles, bars, labels, and areas
            circles.transition().duration(200).style("opacity", 0.8);
            d3.selectAll(".bar, .label, .area").transition().duration(200).style("opacity", 0.8);
        });
...
function createStackedAreaChart(data, circles) {
    const margin = { top: 20, right: 40, bottom: 20, left: 40},
          widthAreaChart = width,
          heightAreaChart = 200 - margin.top - margin.bottom;
    ...
    svg.selectAll("path")
        ...
        .on("mouseout", d => {
            d3.selectAll("path").transition().duration(200).style("opacity", 0.8);
            circles.transition().duration(200).style("opacity", 0.8);
        });
