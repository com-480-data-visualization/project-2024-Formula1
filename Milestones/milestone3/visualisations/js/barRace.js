...
const margin = {top: 20, right: 40, bottom: 40, left: 250}; // Increased left margin for better label placement
...
function resetChart() {
    ...
    g.selectAll(".name-label").remove();
    g.selectAll(".year").remove();
    g.selectAll(".axis").remove(); // Remove axes
}
...
function render(data, metric) {
    ...
    const nameLabels = g.selectAll(".name-label")
        .data(yearData, d => d.name);
    ...
    const yearLabel = g.selectAll(".year")
    ...
    function updateChart() {
        ...
        g.select(".x-axis").selectAll("text").style("fill", "#ffffff");
